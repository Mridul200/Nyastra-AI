...
export default function DocumentGenerator() {
  ...
  const handlePreview = async () => {
    ...
    const prompt = `Draft a strictly formal, government-approved Indian ${selectedDoc.label} with the following details:\n${fieldsText}\n\nUse standard court-recognized legal formatting. Include all mandatory standard clauses, jurisdictional statements, swearing clauses, and formal signature blocks. Return the complete document text only without any preamble.`;
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display font-semibold text-base text-muted-foreground flex items-center gap-3">
                Document Preview
                {preview && !isPreviewLoading && (
                  <Badge 
                    variant="outline" 
                    className="cursor-pointer text-xs font-normal border-primary/40 text-primary hover:bg-primary/10 transition-colors"
                    onClick={() => setIsEditing(!isEditing)}
                  >
                    {isEditing ? "View formatting" : "Edit document"}
                  </Badge>
                )}
              </h2>
              {preview && (
                <button onClick={() => { setPreview(null); setIsEditing(false); }} className="text-muted-foreground hover:text-foreground transition-colors">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
              ...
              {isPreviewLoading ? (
                ...
                isEditing ? (
                  <Textarea
                    ...
                    className="w-full h-full min-h-[400px] bg-transparent border-none resize-none focus-visible:ring-0 p-0 text-sm font-mono leading-relaxed text-foreground"
