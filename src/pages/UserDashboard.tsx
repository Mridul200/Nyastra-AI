...
export default function UserDashboard() {
  ...
  useEffect(() => {
    ...
    if (user) {
      if (user.id === "guest-user-id") {
        ...
        const savedGuestProfile = localStorage.getItem("nyastra_guest_profile");
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (user.id === "guest-user-id") {
        // Handle guest local upload as base64
        const reader = new FileReader();
        reader.onload = () => {
          const base64Url = reader.result as string;
          const updatedProfile = { ...profile, avatar_url: base64Url };
          setProfile(updatedProfile);
          localStorage.setItem("nyastra_guest_profile", JSON.stringify(updatedProfile));
          toast.success("Profile photo updated (Local Guest Mode)!");
        };
  ...
  const handleProfileSave = async (e: React.FormEvent) => {
    ...
    try {
      if (user.id === "guest-user-id") {
        localStorage.setItem("nyastra_guest_profile", JSON.stringify(profile));
        toast.success("Profile updated successfully (Local Guest Mode)!");
      } else {
